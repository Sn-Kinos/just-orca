...
Part of the [just-orca](../../README.md) marketplace. An Orca plugin that opens the focused worktree's commit graph, including recursive submodules, in an Orca browser tab. Requires Orca 1.4+, Git, and Node 20+. No npm dependencies or build step.
...
2. Expand **Development / 개발** below the catalog. Paste this checkout's absolute path into **Development plugin folder path**, then click **Add path**:
   e.g. `/Users/Kinos/Projects/just-orca/plugins/git-log-graph` (this folder, not the repo root).
